...
9. 安防 SQLite 必须明确选择所需字段，禁止 `SELECT *`；同一事件的多张表优先用明确字段的 JOIN 或少量并行查询完成。
10. 查询已足以回答时停止调用工具；除非用户明确要求跨模块分析，不要为了补充背景查询无关数据源。
11. 设备跨日趋势必须分别查询 `vibration.daily_health` 和正式诊断表；逐日阶段、健康指数及 `is_synthetic` 以 `vibration.daily_health` 为准，不得用 `equipment.health_trend.details_json` 替代。
