...
def create_conversation_router(root: Path, store: UserStore) -> APIRouter:
    ...
    async def delete_thread(thread_id: str, http_request: Request) -> Response:
        user = _current_user(http_request)
        if not store.owns_thread(user["user_id"], thread_id):
            raise HTTPException(status_code=404, detail="对话不存在。")
        # 浏览器停止等待只会断开 SSE；这里再主动 abort，避免 checkpoint
        # 删除同步等待仍由模型运行占用的会话锁。
        await service.cancel_thread(user["user_id"], thread_id)
        await asyncio.to_thread(service.delete_thread, user["user_id"], thread_id)
