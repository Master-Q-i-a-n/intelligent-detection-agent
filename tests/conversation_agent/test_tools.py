...
def test_agent_rewrites_rejected_security_select_star_once(tmp_path: Path) -> None:
    (tmp_path / "database").mkdir()
    duckdb.connect(str(tmp_path / "database" / "gas_ai_results.duckdb")).close()
        ...
        responses=[
            AIMessage(
                ...
                tool_calls=[{
                    ...
                    "args": {"sql": "SELECT * FROM security_events"},
                }],
            ),
            AIMessage(
                content="",
    ...
    assert len(tool_messages) == 2
    assert json.loads(str(tool_messages[0].content))["category"] == "readonly_policy"
    assert tool_messages[0].status == "error"
    assert tool_messages[1].status == "success"
